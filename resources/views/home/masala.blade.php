...
<head>
    ...
    <script>
        ...
        // Track ViewContent when combo is viewed
        function trackComboView(comboId) {
        ...
        // Track AddToCart when combo is selected
        function trackComboAddToCart(comboId, qty) {
            qty = qty || 1;
            var combo = combos[comboId];
            if (combo) {
        ...
        function trackCheckoutStart(comboId, total) {
            var combo = combos[comboId];
            if (combo) {
        ...
        function trackPurchase(comboId, total, qty) {
            var combo = combos[comboId];
            if (combo) {
    ...
    <style>
        * {
        ...
        .combo-option.out-of-stock .combo-thumb img {
            filter: grayscale(100%) blur(1px);
            opacity: 0.6;
        }

        .combo-option.out-of-stock .combo-info h4 {
            color: #999;
            text-decoration: line-through;
        }

        .combo-option.out-of-stock .combo-info p {
            color: #999;
        }

        .combo-option.out-of-stock .combo-summary {
            color: #aaa;
        }

        .combo-option.out-of-stock .combo-price {
            color: #e74c3c;
            font-size: 16px;
        }

        .combo-option.out-of-stock .qty-control {
            opacity: 0.3;
            pointer-events: none;
        }

        .combo-option.out-of-stock .qty-control button {
            background: #ddd;
            color: #999;
        }

        .combo-option.out-of-stock .qty-control span {
            color: #999;
        }
        ...
        .combo-option.out-of-stock .combo-right .combo-price .strike-price {
            text-decoration: line-through;
            color: #bbb;
            font-size: 14px;
            margin-right: 8px;
        ...
        /* Combo 1 out of stock in main grid */
        .combo-card.out-of-stock-card {
            opacity: 0.6;
        ...
        .combo-card.out-of-stock-card .combo-image img {
            filter: grayscale(100%) blur(1px);
            opacity: 0.6;
        }

        .combo-card.out-of-stock-card .combo-title {
            color: #999;
            text-decoration: line-through;
        }

        .combo-card.out-of-stock-card .order-btn {
            background: #ccc;
            cursor: not-allowed;
            animation: none;
            pointer-events: none;
        ...
        .combo-card.out-of-stock-card .bn64-price-box span {
            color: #e74c3c;
            text-decoration: line-through;
        }

        .combo-card.out-of-stock-card .delivery {
            background: #ccc;
        }
        ...
        /* Additional styles for gift section */
        .gift-section input:focus {
            outline: none;
            border-color: #28a745;
...
<body>
        ...
        <div class="form-wrapper">
            ...
            <div class="customer-form">
                ...
                <div class="combo-selector">
                    ...
                    <div class="combo-selector">
                        <!-- COMBO 1 -->
                        <label class="combo-option" id="combo1Label">
                            ...
                            <div class="combo-right">
                                ...
                                <div class="qty-control">
                                    ...
                                    <button type="button" onclick="event.stopPropagation();changeCardQty('combo1',-1)">
                                        −
                                    </button>
                        ...
                        <!-- COMBO 2 -->
                        <label class="combo-option popular" id="combo2Label">

                            <div class="popular-tag">
                                সর্বাধিক বিক্রিত
                            </div>

                            <input type="radio" name="combo" value="combo2" checked onchange="selectCombo('combo2')">
                            ...
                            <div class="combo-right">
                                ...
                                <div class="qty-control">
                                    ...
                                    <button type="button" onclick="event.stopPropagation();changeCardQty('combo2',-1)">
                                        −
                                    </button>
                        ...
                        <!-- COMBO 3 -->
                        <label class="combo-option popular" id="combo3Label">
                            ...
                            <div class="combo-right">
                                ...
                                <div class="qty-control">
                                    ...
                                    <button type="button" onclick="event.stopPropagation();changeCardQty('combo3',-1)">
                                        −
                                    </button>
                ...
                <form id="orderForm">
                    ...
                    <div class="form-group">
                        <label>মোবাইল নম্বর * <small>(নম্বর ভুল হলে অর্ডার নিশ্চিত হবে না) </small></label>
                            ...
                            <tr>
                                <td>
                                    <div class="summary-product">
                                        <img src="{{ asset('image/combo2.jpg') }}" id="summaryImage" alt="Achar">
                                        <div class="summary-product-info">
                                            <h5 id="summaryProductName">মসলা কম্বো ২</h5>
                                            <span>Qty × <span id="qtyText">1</span></span>
                                        </div>
                                    </div>
                                </td>
                                <td class="summary-price" id="subtotal">১৩৯৯৳</td>
                            </tr>
                        </tbody>
                    </table>

                    <!-- Delivery Charge Row - Dynamic -->
                    <div class="summary-row" id="deliveryRow">
                        <span>ডেলিভারি চার্জ</span>
                        <span id="deliveryCharge" style="font-weight:700; color: green;">ফ্রি</span>
                    </div>

                    <div class="summary-total">
                        <span>মোট</span>
                        <span id="totalPrice">১৩৯৯৳</span>
                    </div>
    ...
    <script>
        ...
        // ===== FIXED: Track Complete Payment (removes the warning) =====
        function trackCompletePayment(comboId, total, qty, phone, email, name) {
            qty = qty || 1;
            var combo = combos[comboId];
            if (combo) {
                var params = {
                ...
                // Email is also required for Complete Payment (if you have it)
                if (email && email.trim() !== '') {
        ...
        // ===== Track Purchase =====
        function trackPurchase(comboId, total, qty, phone, name) {
            qty = qty || 1;
            var combo = combos[comboId];
            if (combo) {
                var params = {
        ...
        // ============================
        // FACEBOOK EVENT TRACKING - FIXED
        // ============================
        function trackFacebookEvent(eventName, params) {
            if (typeof fbq !== 'undefined') {
                try {
                    // Force currency to BDT and remove any invalid characters
                    if (params.currency) {
                        params.currency = 'BDT';
                    }
        ...
        document.addEventListener("DOMContentLoaded", function () {
            document.querySelector('input[name="combo"][value="combo2"]').checked = true;
            updateCardPrices();
            renderSelectedCombo();

            trackComboView('combo2');
            viewContentTracked = true;
            console.log('✅ TikTok ViewContent tracked on page load');

            trackFacebookEvent('ViewContent', {
                content_name: 'মসলা কম্বো ২',
        ...
        function selectCombo(comboId) {
            if (selectedCombo === comboId) return;
            ...
            trackComboView(comboId);
            trackComboAddToCart(comboId, 1);

            trackFacebookEvent('AddToCart', {
                content_name: combos[comboId].name,
                content_category: 'Combo',
                value: combos[comboId].price,
                currency: 'BDT'
            });
        }
        ...
        function changeCardQty(comboId, change) {
            ...
            trackComboAddToCart(comboId, selectedQty);
        }
        ...
        function updateCardPrices() {
            document.getElementById('combo1Price').innerHTML = combos.combo1.price + ' <small>৳</small>';
            document.getElementById('combo2Price').innerHTML = combos.combo2.price + ' <small>৳</small>';
            document.getElementById('combo3Price').innerHTML = combos.combo3.price + ' <small>৳</small>';

            document.querySelectorAll('.combo-option').forEach(label => {
                label.classList.remove('selected');
            });

            const selectedLabel = document.getElementById(selectedCombo + 'Label');
            if (selectedLabel) {
        ...
        function renderSelectedCombo() {
            ...
            const subtotal = combo.price * selectedQty;
            const delivery = combo.deliveryCharge;
            ...
            if (delivery > 0) {
                ...
                deliveryElement.style.color = '#e74c3c';
            } else {
                deliveryElement.innerHTML = 'ফ্রি';
                deliveryElement.style.color = 'green';
            }
        ...
        function scrollToForm() {
            const combo = combos[selectedCombo];
            const total = (combo.price * selectedQty) + combo.deliveryCharge;

            trackCheckoutStart(selectedCombo, total);
        ...
        function submitForm(event) {
            ...
            const selectedRadio = document.querySelector('input[name="combo"]:checked');
            if (!selectedRadio) {
                Swal.fire({
                    icon: 'error',
                    title: 'কম্বো সিলেক্ট করুন',
                    text: 'অনুগ্রহ করে একটি কম্বো সিলেক্ট করুন।'
                });
                return;
            }

            const comboId = selectedRadio.value;
            const combo = combos[comboId];

            const qtyElement = document.getElementById(comboId + 'Qty');
            ...
            const customerEmail = document.getElementById('email') ? document.getElementById('email').value.trim() : '';
            ...
            fetch("{{ route('order.store') }}", {
                ...
                .then(response => {
                    // ===== TRACK COMPLETE PAYMENT (removes the warning) =====
                    trackCompletePayment(comboId, total, qty, customerPhone, customerEmail, customerName);
                    console.log('✅ TikTok CompletePayment tracked');

                    // ===== TRACK PURCHASE =====
                    trackPurchase(comboId, total, qty, customerPhone, customerName);
                    console.log('✅ TikTok Purchase tracked - Total: ' + total);

                    // ===== TRACK FACEBOOK PURCHASE - FIXED =====
